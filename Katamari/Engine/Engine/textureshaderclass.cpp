...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutShaderErrorMessage(errorMessage, hwnd, vsFilename);
		}
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutShaderErrorMessage(errorMessage, hwnd, psFilename);
		}
...
void TextureShaderClass::OutShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
